...
下载：[Nvidia Driver驱动](https://www.nvidia.com/download/index.aspx)，[CUDA Toolkit](https://developer.nvidia.com/cuda-downloads)，[cuDNN](https://developer.nvidia.com/cudnn-downloads)
...
3. `firewall-cmd --query-port=1234/tcp`，如果显示 no，进行下一步
4. 添加指定需要开放的端口：`firewall-cmd --add-port=1234/tcp --permanent`

    重载入添加的端口：`firewall-cmd --reload`

    查询指定端口是否开启成功：`firewall-cmd --query-port=1234/tcp`

    (移除指定端口：`firewall-cmd --permanent --remove-port=1234/tcp`)
...
5.`pip` 换用国内镜像源，如清华源`https://pypi.tuna.tsinghua.edu.cn/simple`，中科大源`https://pypi.mirrors.ustc.edu.cn/simple`，豆瓣源`http://pypi.douban.com/simple`

```shell
pip install <package_name> -i https://mirrors.aliyun.com/pypi/simple # 暂时换源
pip config set global.index-url https://mirrors.aliyun.com/pypi/simple # 永久换源
